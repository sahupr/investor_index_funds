...
def download_history():
    ...
    for t in open(f'{dirname(dirname(abspath(__file__)))}/data/{sys.argv[2]}').readlines():
        ticker = t.strip()
        try:
            ...
            pickle.dump(data, open(f'{dirname(dirname(abspath(__file__)))}/data/eod_history/{ticker}.pkl', 'wb+'))
            print(ticker)
        except:
...
def record_shares_outstanding():
    ...
    pickle.dump(outstanding, open(f'{dirname(dirname(abspath(__file__)))}/data/shares_outstanding.pkl', 'wb+'))
